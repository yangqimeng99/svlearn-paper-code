...
do
	cat ${sample_list} | while read sample
		...
		awk '
                ...
                }' OFS="\t" ../RF-models_${num}feature/lack.${sample}.RF-model/val_result.txt > transposed_val_result.txt
		...
		' ../log/lack.${sample}.${num}feature.training_model.log > ${sample}_${num}fea_best_parameters.tsv
	done
done
