sample="Charolais"
sv_feature="../../../05.feature_extraction/Cattle_SV_Set/sv_feature.tsv"
true_sv_set="../../../02.long-reads_mapping_SVcalling/cattle/Cattle_SV_Set.vcf"
predicted_vcf_path=".."
...
for TYPE in all DEL.nonTR INS.nonTR DEL.TR INS.TR
do
	bcftools view -i "ID=@sv.list/${TYPE}.sv.list" \
		-s ${sample} ${true_sv_set}  > sv.true_vcf/${sample}.${TYPE}.true_GT.vcf
done

for tool in paragraph bayestyper graphtyper svtyper svlearn_24 svlearn_18
do
...
csvtk concat -t */${sample}.*.bench.row.tsv > benchmark_TR.tsv
