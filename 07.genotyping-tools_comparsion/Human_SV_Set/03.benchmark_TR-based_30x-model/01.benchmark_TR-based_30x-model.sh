...
for tool in paragraph bayestyper graphtyper svtyper
do
...
# SVLearn 30x-model
for tool in svlearn_24 svlearn_18
do
        mkdir -p ${tool}/01.vcf ${tool}/02.bench
        for coverage in 30 20 10 5
        do
                ...
                do
			bcftools view -i "ID=@sv.list/${TYPE}.sv.list" \
                                ${predicted_vcf_path}/${sample}.${tool}.${coverage}.30x-model.vcf.gz \
                                -O z -o ${tool}/01.vcf/${sample}.${coverage}.${TYPE}.vcf.gz
...
csvtk concat -t */${sample}.*.bench.row.tsv > benchmark_TR-based_30x-model.tsv
